...
This project added searchable OCR captions to [Gordon Landreth Photography](https://gordon-landreth-photography.arts-link.com/), a static site hosting my grandfather's photography from 1931-1990s. You can [try the search interface](https://gordon-landreth-photography.arts-link.com/search/) to explore 3,400+ scanned photo album pages.
...
Processing 3,416 images takes time. My **separate index rebuild script** (`rebuild_search_index.py`) reads existing OCR JSONs and regenerates the search index in 3-5 seconds, which is invaluable for testing search functionality and fixing metadata bugs without rerunning OCR.
...
The gallery already used PhotoSwipe for album pages. Reusing it for search results meant consistent UX with full keyboard navigation, swipe gestures, and caption display with no new dependencies.
...
When searching for an album name, both the album and individual pages appeared in results, which was confusing. I split results into two groups:
...
Family members are now finding photos they forgot existed. They can click a thumbnail, browse results in full-screen lightbox, navigate with arrow keys, and see high-quality captions from the vision model.

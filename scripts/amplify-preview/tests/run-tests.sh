...
check t1 "builds against the preview host, not production" yes "$(contains "$(cat "$MOCK_STATE/calls.log")" 'HUGO_BASEURL')"
check t1 "HUGO_BASEURL is the pr-12 preview URL" yes "$(contains "$(cat "$MOCK_STATE/calls.log")" '"HUGO_BASEURL":"https://pr-12.d123abc.amplifyapp.com/"')"
check t1 "still carries the policy override" yes "$(contains "$(cat "$MOCK_STATE/calls.log")" '"PUBLIC_POSTHOG_KEY":""')"
...
check t2 "sets HUGO_BASEURL on the existing branch too" yes "$(contains "$(cat "$MOCK_STATE/calls.log")" '"HUGO_BASEURL":"https://pr-12.d123abc.amplifyapp.com/"')"
